...
from management.models import Product, Sale
from management.forms import SaveProductForm
...
class Dashboard(LoginRequiredMixin, generic.View):
  ...
  def get(self, request, *args, **kwargs):
    ...
    try:
      total_sale = Sale.objects.filter(product__id__in = fuels).aggregate(Sum('total_amount'))['total_amount__sum']
...
class FuelListView(LoginRequiredMixin, generic.ListView):
  model = Product
  template_name = 'fuel_management/fuel_list.html'
...
class FuelDetailView(LoginRequiredMixin, generic.DetailView):
  model = Product
  template_name = 'fuel_management/fuel_detail.html'
...
class FuelCreateUpdateView(LoginRequiredMixin, generic.View):
  template_name = 'fuel_management/manage_fuel.html'
...
class FuelDeleteView(LoginRequiredMixin, generic.View):
  ...
  def delete(self, request, pk=None, *args, **kwargs):
    ...
    if pk is None:
      ...
      fuel_instance = get_object_or_404(Product, pk=pk, category='FUEL')
      try:
